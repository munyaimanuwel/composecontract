...
**Repo today:** `https://github.com/munyaimanuwel/composecontract` (still private; product name is StackContract)  
...
- [ ] GitHub **repository rename** to `stackcontract`  
...
- [ ] Confirm `main` builds locally: `dotnet test StackContract.sln -c Release`
- [ ] Confirm tool pack: `dotnet pack StackContract.sln -c Release -o ./nupkg` then dry-run install of `stackcontract`

---

## 1. Scrub before Public (secrets / PII)

- [ ] `.gitignore` covers `.env`, `*.user`, `nupkg/`, `bin/`, `obj/`
- [ ] No real secrets in git history on `main` (sample `.env.example` keys only)
- [ ] No employer names, private hostnames, Tailscale IPs, or customer paths in README/samples
- [ ] LICENSE is MIT and matches intent
- [ ] README states: *Not affiliated with Docker, Inc.*
- [ ] Repo **Description** updated (e.g. “Local-first .NET stack/config contract checker”)
- [ ] Optional: Topics — `dotnet`, `docker-compose`, `config`, `cli`, `github-actions`
...
- [ ] Confirm anonymous browse works: `https://github.com/munyaimanuwel/stackcontract` (or current slug if rename deferred)
- [ ] Pin the repo on your GitHub profile (if not already)
- [ ] Update profile README link if slug changed (drop “pending rename” note)
...
- [ ] NuGet.org account + API key ready (personal, not work)
- [ ] Package IDs free: `stackcontract` tool + `StackContract.*` libraries
- [ ] Set `PackageProjectUrl` / `RepositoryUrl` to final public URL
...
- [ ] Clear free vs paid boundary (MIT core stays free; Pro = kits / CI extras / templates — no phone-home in Core/Engine)
- [ ] Landing copy + pricing
- [ ] Delivery: private repo, download, or license key — pick one and document
- [ ] Link Pro Kit from public README (“Sponsors / Pro” section)
...
| Repo slug → `stackcontract` | Manual |
| Visibility → Public | Manual |
| NuGet publish | Blocked on Public |
| Pro Kit | Blocked on NuGet |
